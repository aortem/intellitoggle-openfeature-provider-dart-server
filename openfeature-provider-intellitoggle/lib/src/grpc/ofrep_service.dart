...
import 'package:openfeature_provider_intellitoggle/src/gen/ofrep.pb.dart'
    as ofrep;
import 'package:openfeature_provider_intellitoggle/src/gen/ofrep.pbgrpc.dart'
    as ofrepgrpc;
...
class OfrepService extends ofrepgrpc.OfrepServiceBase {
  ...
  final InMemoryProvider? inMemoryProvider;
  final String apiKey;
  ...
  Future<ofrep.EvaluationResponse> getEvaluation(
    ServiceCall call,
    ofrep.EvaluationRequest request,
  ) async {
    ...
    if (inMemoryProvider != null && !inMemoryProvider!.hasFlag(flagKey)) {
    ...
    try {
      ...
      switch (kind) {
        ...
        case ofrep.Value_Kind.jsonObject:
          result = await provider.getObjectFlag(
            flagKey,
            $convert.jsonDecode(dv.jsonObject) as Map<String, dynamic>,
            context: ctx,
          );
          break;
      ...
      throw GrpcError.invalidArgument(e.toString());
    } on ApiException catch (e) {
      throw GrpcError.unavailable(e.toString());
    } catch (e) {
  ...
  ofrep.Value _toValue(dynamic v) {
    ...
    if (v is bool)
      ...
      val.intValue = $fixnum.Int64(v);
    else if (v is double)
      val.doubleValue = v;
    else if (v is Map<String, dynamic>)
      val.jsonObject = $convert.jsonEncode(v);
    else
    ...
    if (hsSecret != null && hsSecret.isNotEmpty) {
      try {
        ...
        final verifier = HmacSignatureVerifier(
          secret: $convert.utf8.encode(hsSecret),
        );
        ...
        if (expectedAud != null || expectedIss != null) {
          ...
          if (expectedAud != null && payload['aud'] != expectedAud)
            return false;
          if (expectedIss != null && payload['iss'] != expectedIss)
            return false;
        }
        return true;
      } catch (_) {
